...
pub const Resampler = struct {
    ...
    pub fn process(self: *Resampler, in_buf: []const i16, out_buf: []i16) !Result {
        ...
        if (in_buf.len == 0 or out_buf.len == 0) {
        ...
        if (self.in_rate == self.out_rate) {
            ...
            return .{
                .in_consumed = @intCast(copy_samples),
                .out_produced = @intCast(copy_samples),
            };
        }

        const res = try self.inner.processInterleavedInt(in_buf, out_buf);
        return .{
            .in_consumed = res.in_consumed,
            .out_produced = res.out_written,
        };
    }
};
